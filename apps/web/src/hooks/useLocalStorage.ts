...
export const getLocalStorageItem = <T, E>(
  ...
  try {
    ...
    if (fallback === undefined) throw new Error(`Failed to decode localStorage key "${key}"`);
    // Schema changed — salvage stored values by merging with defaults
    try {
      const raw = JSON.parse(item) as Record<string, unknown>;
      if (raw && typeof raw === "object" && !Array.isArray(raw)) {
        const merged = { ...fallback, ...raw } as T;
        // Round-trip through encode to strip unknown fields and validate
        const reEncoded = encode(schema, merged);
        return decode(schema, reEncoded);
      }
    } catch {
      // Recovery failed — caller will use fallback
    }
    return null;
  }
};
...
export function useLocalStorage<T, E>(
  ...
  const [storedValue, setStoredValue] = useState<T>(() => {
    try {
      const item = getLocalStorageItem(key, schema, initialValue);
  ...
  useEffect(() => {
    if (prevKeyRef.current !== key) {
      ...
      try {
        const newValue = getLocalStorageItem(key, schema, initialValue);
